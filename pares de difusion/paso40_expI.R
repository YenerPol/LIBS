...
g %>% plotly::ggplotly()
## datos para origin
df <- data.frame(Er = Er, Zr = Zr, Nb = Nb) %>% rowid_to_column() %>% mutate(distancia = rowid*40)
write_csv(df, file = "C:/Users/gomez/Documents/MEGA/sabato/IS - Tesis/Par 950ºC/nuevos datos/expI_completo.csv")
# caption <- paste("Lineas elegidas:","/n",
#                  "Er: ",pico_Er,"/n", 
#                  "Zr: ",pico_Zr,"/n", 
#                  "Nb: ",pico_Nb,"/n", sep = '')
...
df_Er_Zr <- df %>% 
        filter(distancia >= 6560 & distancia <=9360) %>% 
        select(cuentas) %>% 
        rowid_to_column() %>% 
...
FUN.perfil(df_Er_Zr) %>% plotly::ggplotly()
write_csv(df_Er_Zr, file = "C:/Users/gomez/Documents/MEGA/sabato/IS - Tesis/Par 950ºC/nuevos datos/expI_Zr.csv")
...
df_Er_Zr20Nb <- df %>% 
        filter(distancia >= 880 & distancia <=4680) %>% 
        select(cuentas) %>% 
        rowid_to_column() %>% 
        mutate(cuentas =cuentas,
               distancia = rowid*40)
...
write_csv(df_Er_Zr20Nb, file = "C:/Users/gomez/Documents/MEGA/sabato/IS - Tesis/Par 950ºC/nuevos datos/expI_Zr20Nb.csv")
...
write_csv(df_Er_Zr20Nb[,c('distancia','cuentas')], file = paste(dir,'Er_Zr20Nb_expI.txt',sep = ''), col_names = FALSE)
