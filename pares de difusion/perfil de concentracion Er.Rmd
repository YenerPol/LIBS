...
#pico_Zr <- 339.0371
pico_Zr <- 357.0441
FUN.plot.pico(pico = pico_Zr, w = 0.5) #%>% plotly::ggplotly()
```

## Selección pico de Nb

```{r}
pico_Nb <- c(322.3791, 309.2509, 319.32, 316.25)
FUN.plot.pico(pico = pico_Nb[1] , w = 0.5) #%>% plotly::ggplotly()
```
...
#g %>% plotly::ggplotly() #+ annotate(geom = 'text', x = 250, y = 0.020, label = caption) 
```

```{r}
ggsave('./outputs/perfil_datos_ExpI.jpg', device = 'jpg',
       plot = last_plot(),
       width = 24,
       height = 12,
       units = "cm")
