...
#### Data ####
# Data proveCrente de (2) Preprop.R
load('./Data/Data.RData')

# Class	 C	    Si  	S	    P	    Mn	    Cr	    Cr	    Mo      V   	Cu
...
#### Recta 1 ####
Int <- X %>% select(`283.38989`:`283.48524`) %>% apply(1,sum)
df <- data.frame(Int, Y)

df %>% ggplot(aes(Int, Cr)) +
        geom_point() +
        geom_smooth(method = 'lm')

linear.model <- lm(Cr ~ Int, df)
summary(linear.model)

Int.test <- X.test %>% select(`283.38989`:`283.48524`) %>% apply(1,sum)

Y.test$predicted <- predict(linear.model, newdata = data.frame(Int = Int.test))

## calcular MSE y MAE general
Y.test <- Y.test %>% 
        mutate(dif = Cr - predicted)

mse <-  mean((Y.test$dif)^2)
mae <-  mean(abs(Y.test$dif))
rmse = sqrt(mse)
R2 = 1-(sum((Y.test$dif)^2)/sum((Y.test$Cr -mean(Y.test$Cr))^2))
...
df2 <- split(df, df$Cr) %>% map_dfr(~ apply(.x,2,mean))

df2 %>% ggplot(aes(Int , Cr)) + 
        geom_point(size = 6) + 
        geom_smooth(method = 'lm') +
...
Y.test <- Y.test %>% 
        mutate(dif2 = Cr - predicted2)

mse2 <-  mean((Y.test$dif2)^2)
mae2 <-  mean(abs(Y.test$dif2))
rmse2 = sqrt(mse2)
R2.2 = 1-(sum((Y.test$dif2)^2)/sum((Y.test$Cr -mean(Y.test$Cr))^2))
...
Y.test %>% 
        ggplot(aes(x = Cr, y = predicted2)) +
                geom_violin(aes(fill = factor(Cr))) + 
...
Y.test %>% 
        ggplot(aes(x = Cr, y = predicted2)) +
        geom_boxplot(aes(fill = factor(Cr))) + 
        xlab("Concentracion Real") +
        ylab("Concentracion Predicha") +
        geom_abline(intercept = 0, slope = 1, color="red", size=0.75) +
        geom_hline(yintercept = c, linetype = "dashed") +
